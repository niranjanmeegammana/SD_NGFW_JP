...
log_path = current_folder +slash + "results" + slash +'log_m1.csv'
log_path2= current_folder +slash + "results" + slash +'log_m2.csv'
    ...
    if b_model_loading == 0:  # Model is loaded
        if s_running_model[-2:] == "20":
            if index20 < max_rec:
                ...
                y_pred = active_model.predict(r, verbose=0)[0]  # Extract single value
                index20 += 1 
                if y_test[index20] == 1:
                    actual_attack += 1
                    if y_pred == 1:
