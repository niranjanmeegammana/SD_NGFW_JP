...
log_path = current_folder +slash + "results" + slash +'log.csv'
...
false_positive_rate = 0.0  # FP / (FP + TN)
...
def initialize():
    ...
    global t_send_data, t_inference_rate
    global scaler20, scaler40
    global debug_str
    
    b_model_loading=1
    t_send_data=0.2 #1
    t_inference_rate=1#1 4
...
# Function that simulates continuous activity
async def change_inference_rate():
    ...
    await asyncio.sleep(t_inference_rate)  

'''
actual_attack: Total actual attack samples
predicted_attack_as_attack: Correctly predicted attack instances
predicted_attack_as_benign: Attack instances incorrectly predicted as benign
actual_benign: Total actual benign samples
predicted_benign_as_benign: Correctly predicted benign instances
predicted_benign_as_attack: Benign instances incorrectly predicted as attack
'''
...
async def xtest_data():
    ...
    global actual_benign, predicted_benign_as_benign, predicted_benign_as_attack

    #if (testing_on==1): return
    ...
    max_rec = len(y_test)
    print("00000000000000000000000000000000000")
    while (index20 < max_rec) and (index40 < max_rec):
...
def run_test_data():
    """Runs test_data in a separate thread."""
    test_data()
