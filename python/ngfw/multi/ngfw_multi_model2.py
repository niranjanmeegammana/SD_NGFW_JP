...
log_path = current_folder +slash + "results" + slash +'log_m2_1.csv'
log_path2= current_folder +slash + "results" + slash +'log_m2_2.csv'
...
def load_sd_models():
    global modelpath, debug_str, active_model
    global deep_40, shallow_40, deep_20, shallow_20
    global sd_models  # Assuming this is defined elsewhere
    
    debug_str = "loading models"
    ...
    for i in range(4):
        model = sd_models[i + 1]
        print(ts, "Loading model", model)
        
        model_path = modelpath + model + '_model_ANN5_hp.keras'
        t1 = time.time()  # Record start time
        
        active_model = load_model(model_path)
        
        if model == "deep_40":
            deep_40 = active_model
        elif model == "shallow_40":
            shallow_40 = active_model
        elif model == "deep_20":
            deep_20 = active_model
        else:
            shallow_20 = active_model

        t2 = time.time()  # Record end time
        append_log(i + 1, model, 0, t2 - t1)
        
        ts = get_time()    
        print(ts, "Loaded", model)

    s_running_model = sd_models[1]
    s_next_model = s_running_model
    active_model = deep_40
    b_model_active = 1
    
    ts = get_time()    
    print(ts, "Loaded", sd_models[1])
    
    b_model_loading = 0

def create_scaler(features, fnx):
    global df_test, s_next_model
    df_sample = df_test.sample(5000, random_state=42) 
...
''' 
def xswitch_model(s_next_model):
    ...
    t2 = time.time()  # Record start time
...
def switch_model(s_next_model):
    ...
    print(ts, "Switching model", s_next_model)  # ✅ Fixed syntax
    
    b_model_loading = 1
    switch_scaler_data(s_next_model)    
    t1 = time.time()  # Record start time
    
    if s_next_model == "deep_40":
        active_model = deep_40
    elif s_next_model == "shallow_40":
        active_model = shallow_40
    elif s_next_model == "deep_20":
        active_model = deep_20
    else:  # ✅ Colon added
        active_model = shallow_20
...
async def test_data_multi():   
    ...
    global index20, index40, index20_m, index40_m 
    global done_20, done_40
    global log_path2
    ...
    if b_model_loading == 0:  # Model is loaded
        if s_running_model[-2:] == "20":
            index20_m = min(index20 + m, max_rec)
            if index20_m < max_rec:
                print(s_running_model[-2:],index20, index20_m)  
                # Extract m records for inference
                r = X_test20[index20 : index20_m].reshape(m, -1)
                # Perform inference on m records
                t1 = time.time()
                y_pred = active_model.predict(r, verbose=0).flatten()
                t2 = time.time()
                append_log2(s_running_model, index20_m,speed, t2 - t1)
                # Ensure it's a 1D array
                # Extract actual values
                y_actual = y_test[index20 : index20_m]
                # Compare each actual and predicted value

                for actual, predicted in zip(y_actual, y_pred.round().astype(int)):  # Round predictions for classification
                    if actual == 1:
                        actual_attack += 1
                        if predicted == 1:
            ...
            index40_m = min(index40 + m, max_rec)
            if index40_m < max_rec:
                print(s_running_model[-2:],index40, index40_m)
               
                # Extract m records for inference
                r = X_test40[index40 : index40_m].reshape(m, -1)
                t1 = time.time()
                # Perform inference on m records
                y_pred = active_model.predict(r, verbose=0).flatten()
                t2 = time.time()
                append_log2(s_running_model, index40_m,speed, t2 - t1)
                  # Ensure it's a 1D array
                # Extract actual values
                y_actual = y_test[index40 : index40_m]
                # Compare each actual and predicted value
    ...
    if (b_model_active==1) and (index20_m >= max_rec) and (index40_m >= max_rec):
        # Calculate Metrics
...
async def change_inference_rate():
    ...
    if b_model_loading == 0:
        ...
        if num % 2 == 0:  # Check if num is divisible by 5
            if (done_20==0) and (done_40==0):    
                speed = random.randint(1, 100) % 4 + 1         

            if (done_20==0) and (done_40==1):
                speed = random.randint(1, 100) % 2 + 3 
                
            if (done_20==1) and (done_40==0):
                speed = random.randint(1, 100) % 2 + 1 
